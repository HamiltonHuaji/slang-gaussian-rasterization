...
def vertex_and_tile_shader(
    ...
    with torch.no_grad(), device:
        ...
        unsorted_keys = torch.zeros((total_size_index_buffer,), dtype=torch.int64) # type: ignore
        unsorted_gauss_idx = torch.zeros((total_size_index_buffer,), dtype=torch.int32) # type: ignore
        slang_modules.tile_shader.generate_keys(
            xyz_vs=xyz_vs,
        ...
        ).launchRaw(blockSize=(256, 1, 1), gridSize=(math.ceil(n_points/256), 1, 1))
        ...
        tile_ranges = torch.zeros((render_grid.grid_height*render_grid.grid_width, 2), dtype=torch.int32)
        slang_modules.tile_shader.compute_tile_ranges(
            sorted_keys=sorted_keys,
            out_tile_ranges=tile_ranges
        ).launchRaw(blockSize=(256, 1, 1), gridSize=(math.ceil(total_size_index_buffer/256), 1, 1))
...
class VertexShader(torch.autograd.Function):
    ...
    def forward(
        ...
        n_points = xyz_ws.shape[0]
        tiles_touched = torch.zeros((n_points), device=device, dtype=torch.int32)
        rect_tile_space = torch.zeros((n_points, 4), device=device, dtype=torch.int32)
        radii = torch.zeros((n_points), device=device, dtype=torch.int32)
        
        xyz_vs = torch.zeros((n_points, 3), device=device, dtype=torch.float)
        inv_cov_vs = torch.zeros((n_points, 2, 2), device=device, dtype=torch.float)
        rgb = torch.zeros((n_points, 3), device=device, dtype=torch.float)

        slang_modules.vertex_shader.vertex_shader(
            xyz_ws=xyz_ws,
            rotations=rotations,
            scales=scales,
            sh_coeffs=sh_coeffs,
    ...
    def backward(ctx, grad_tiles_touched, grad_rect_tile_space, grad_radii, grad_xyz_vs, grad_inv_cov_vs, grad_rgb):
        ...
        slang_modules.vertex_shader.vertex_shader.bwd( # type: ignore
