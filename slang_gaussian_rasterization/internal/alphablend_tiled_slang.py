...
def render_alpha_blend_tiles_slang_raw(
    xyz_ws, rotations, scales, opacity, 
    sh_coeffs, active_sh,
    ...
    sorted_gauss_idx, tile_ranges, radii, xyz_vs, inv_cov_vs, rgb = vertex_and_tile_shader(
        xyz_ws,
        rotations,
        scales,
        sh_coeffs,
    ...
    image_rgb: torch.Tensor = AlphaBlendTiledRender.apply(
        sorted_gauss_idx,
        tile_ranges,
        xyz_vs,
        inv_cov_vs,
...
class AlphaBlendTiledRender(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx, sorted_gauss_idx, tile_ranges,
        xyz_vs, inv_cov_vs, opacity, rgb, render_grid, device: torch.device
    ):
        with device:
            output_img = torch.zeros((render_grid.image_height, render_grid.image_width, 4), device=device)
            n_contributors = torch.zeros((render_grid.image_height, render_grid.image_width, 1), dtype=torch.int32, device=device)
            ...
            splat_kernel_with_args.launchRaw(
                blockSize=(render_grid.tile_width, render_grid.tile_height, 1),
                gridSize=(render_grid.grid_width, render_grid.grid_height, 1)
            )
